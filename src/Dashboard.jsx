...
function MonthlyRhythm({ tracker, risks, controls }) {
  const openTasks = tracker.items.filter(item => !['Done', 'Skipped'].includes(item.status))
  const recurring = tracker.items.filter(item => item.frequency || item.type === 'Recurring')
  const highRisks = risks.items.filter(item => item.probability === 'High')
  const gaps = controls.items.filter(item => ['Partial', 'Not In Place'].includes(item.status))
  return <><MetricBand metrics={[{ label: 'Open activities', value: openTasks.length, onClick: () => {} }, { label: 'Overdue', value: tracker.byStatus.Overdue || 0, tone: 'critical', onClick: () => {} }, { label: 'High probability', value: highRisks.length, tone: 'critical', onClick: () => {} }, { label: 'Control gaps', value: gaps.length, tone: 'attention', onClick: () => {} }, { label: 'Recurring', value: recurring.length, onClick: () => {} }, { label: 'Activities done', value: tracker.byStatus.Done || 0, tone: 'good', onClick: () => {} }]} /><div className="two-columns"><Panel title="Upcoming Deadlines" source="Governance Tracker"><TaskRows items={tracker.upcoming} /></Panel><Panel title="High Probability" source="Unified Risk Register"><TaskRows items={highRisks.map(item => ({ ...item, activityId: item.riskId, dueDate: item.reviewDate, status: item.controlStatus }))} /></Panel></div><Panel title="Controls Register" source="Controls Register"><TaskRows items={gaps.map(item => ({ ...item, activityId: item.controlId, dueDate: item.reviewDate, status: item.status }))} /></Panel></>
}

function Safeguarding({ tracker, risks, controls }) {
  const taskRows = tracker.items.filter(item => item.domain === 'Safeguarding')
  const riskRows = risks.items.filter(item => item.domain === 'Safeguarding')
  const controlRows = controls.items.filter(item => item.domain === 'Safeguarding')
  const openTasks = taskRows.filter(item => !['Done', 'Skipped'].includes(item.status))
  return <><section className="state-band"><div><strong>{openTasks.length}</strong><span>Open activities</span></div><div><strong>{taskRows.filter(item => item.status === 'Overdue').length}</strong><span>Overdue</span></div><div><strong>{riskRows.length}</strong><span>Risks</span></div><div><strong>{controlRows.filter(item => item.status === 'Active').length}</strong><span>Controls active</span></div></section><div className="two-columns"><Panel title="Governance Tracker" source="Governance Tracker"><TaskRows items={taskRows} /></Panel><Panel title="Risk Register" source="Unified Risk Register"><TaskRows items={riskRows.map(item => ({ ...item, activityId: item.riskId, dueDate: item.reviewDate, status: item.controlStatus }))} /></Panel></div><Panel title="Controls Register" source="Controls Register"><TaskRows items={controlRows.map(item => ({ ...item, activityId: item.controlId, dueDate: item.reviewDate, status: item.status }))} /></Panel></>
}

function RoPA({ ropa, filter, onFilter }) {
  const rows = ropa.items.filter(item => filter === 'all' || item.flag === filter)
  const flags = [...new Set(ropa.items.map(item => item.flag).filter(Boolean))]
  return <><MetricBand metrics={[{ label: 'Processing activities', value: ropa.total, onClick: () => onFilter('all') }, { label: 'Reviewed', value: ropa.byFlag.Reviewed || 0, tone: 'good', onClick: () => onFilter('Reviewed') }, { label: 'LIA needed', value: ropa.byFlag['LIA needed'] || 0, tone: 'attention', onClick: () => onFilter('LIA needed') }, { label: 'Review due', value: ropa.byFlag['Review due'] || 0, onClick: () => onFilter('Review due') }]} /><FilterBar value={filter} onChange={onFilter} options={[['all', 'All processing'], ...flags.map(flag => [flag, flag])]} /><Panel title="Register of Processing Activities" source="RoPA"><Pager items={rows}>{visible => <div className="data-table ropa-table"><div className="table-head"><span>Processing activity</span><span>Data subjects</span><span>Personal data</span><span>Purpose</span><span>Lawful basis</span><span>Systems</span><span>Retention</span><span>Flag</span></div>{visible.map(item => <div className="table-row" key={item.id}><NotionLink item={item}>{item.name}</NotionLink><span>{item.subjects || '—'}</span><span>{item.personalData || '—'}</span><span>{item.purpose || '—'}</span><span>{item.basis || '—'}</span><span>{item.systems || '—'}</span><span>{item.retention || '—'}</span><Badge>{item.flag}</Badge></div>)}</div>}</Pager></Panel></>
}

function ITTools({ tools, filter, onFilter }) {
  const rows = tools.items.filter(item => filter === 'all' || filter === 'critical' && item.criticality === 'Critical' || filter === 'dpa' && item.dpa === 'Pending' || filter === 'mfa' && item.mfa !== 'Enabled')
  const critical = tools.items.filter(item => item.criticality === 'Critical').length
  const dpaPending = tools.items.filter(item => item.dpa === 'Pending').length
  const mfaEnabled = tools.items.filter(item => item.mfa === 'Enabled').length
  return <><MetricBand metrics={[{ label: 'Tools and services', value: tools.total, onClick: () => onFilter('all') }, { label: 'Critical suppliers', value: critical, tone: 'critical', onClick: () => onFilter('critical') }, { label: 'DPA pending', value: dpaPending, tone: 'critical', onClick: () => onFilter('dpa') }, { label: 'MFA enabled', value: mfaEnabled, tone: 'good', onClick: () => onFilter('mfa') }]} /><FilterBar value={filter} onChange={onFilter} options={[['all', 'All tools'], ['critical', 'Critical'], ['dpa', 'DPA pending'], ['mfa', 'MFA not enabled']]} /><Panel title="Access Matrix" source="Access Matrix"><Pager items={rows}>{visible => <div className="data-table tools-table"><div className="table-head"><span>Tool / supplier</span><span>Category</span><span>Owner</span><span>Criticality</span><span>DPA</span><span>MFA</span><span>Next review</span></div>{visible.map(item => <div className="table-row" key={item.id}><NotionLink item={item}>{item.name}</NotionLink><span>{item.category || '—'}</span><span>{item.owner || '—'}</span><Badge>{item.criticality}</Badge><Badge>{item.dpa}</Badge><Badge>{item.mfa}</Badge><time>{formatDate(item.reviewDate)}</time></div>)}</div>}</Pager></Panel></>
}
...
export default function Dashboard() {
  ...
  const load = useCallback(async () => {
    ...
    const next = Object.fromEntries(results.map((result, index) => [keys[index], result.status === 'fulfilled' ? result.value : { total: 0, items: [], byStatus: {}, byDomain: {}, byFlag: {} }]))
    if (results.slice(0, 4).every(result => result.status === 'rejected')) setError('Could not load Notion data')
  ...
  const scoped = useMemo(() => {
    ...
    const ropaItems = (data.ropa?.items || []).filter(onlyPerson)
    const toolItems = (data.tools?.items || []).filter(onlyPerson)
    return {
      ...
      ropa: { ...data.ropa, total: ropaItems.length, items: ropaItems, byFlag: ropaItems.reduce((result, item) => item.flag ? { ...result, [item.flag]: (result[item.flag] || 0) + 1 } : result, {}) },
      tools: { ...data.tools, total: toolItems.length, items: toolItems },
    }
  }, [data, person])
  const tabs = [['overview', 'Overview'], ['tracker', 'Governance Tracker'], ['risks', 'Risk Register'], ['controls', 'Controls'], ['monthly', 'Monthly Rhythm'], ['safeguarding', 'Safeguarding'], ['documents', 'Document Library'], ['ropa', 'RoPA'], ['tools', 'IT Tools']]
  const open = (nextTab, nextFilter = 'all') => { setTab(nextTab); setFilter(nextFilter) }
  const view = tab === 'overview' ? <Overview {...scoped} onOpen={open} /> : tab === 'tracker' ? <GovernanceTracker tracker={scoped.tracker} filter={filter} onFilter={setFilter} /> : tab === 'risks' ? <RiskRegister risks={scoped.risks} filter={filter} onFilter={setFilter} /> : tab === 'controls' ? <Controls controls={scoped.controls} filter={filter} onFilter={setFilter} /> : tab === 'monthly' ? <MonthlyRhythm tracker={scoped.tracker} risks={scoped.risks} controls={scoped.controls} /> : tab === 'safeguarding' ? <Safeguarding tracker={scoped.tracker} risks={scoped.risks} controls={scoped.controls} /> : tab === 'documents' ? <DocumentLibrary documents={scoped.documents} filter={filter} onFilter={setFilter} /> : tab === 'ropa' ? <RoPA ropa={scoped.ropa} filter={filter} onFilter={setFilter} /> : <ITTools tools={scoped.tools} filter={filter} onFilter={setFilter} />
